...
save_config() {
    local port=$1
    if [[ ! -f ~/domains/niboy0088.niboy.top/vless/config.json ]]; then
        uuid=$(generate_uuid)
        cat <<EOL > ~/domains/niboy0088.niboy.top/vless/config.json
{
    "uuid": "$uuid",
    "port": $port
}
EOL
        ...
        jq --arg port "$port" '.port = ($port | tonumber)' ~/domains/niboy0088.niboy.top/vless/config.json > ~/domains/niboy0088.niboy.top/vless/config_tmp.json && mv ~/domains/niboy0088.niboy.top/vless/config_tmp.json ~/domains/niboy0088.niboy.top/vless/config.json
...
deploy_vless() {
    ...
    npm install
    # 启动vless项目
    ~/.npm-global/bin/pm2 start ~/domains/niboy0088.niboy.top/vless/app.js --name vless
    # 保存pm2进程状态
    ...
    echo -e "域名: ${GREEN}niboy0088.niboy.top${NC}"
    echo -e "vless进程维护定时任务脚本: ${GREEN}cd ~/domains/niboy0088.niboy.top/vless && ./check_vless.sh${NC}"
    echo -e "VLESS节点信息: ${GREEN}vless://${uuid}@niboy0088.niboy.top:${port}?flow=&security=none&encryption=none&type=ws&host=niboy0088.niboy.top&path=/&sni=&fp=&pbk=&sid=#$USER.serv00.vless${NC}"
}
...
main() {
    ...
    if [ "$port_provided" = true ]; then
        ...
        if [[ -f config.json ]]; then
            ...
            echo -e "UUID: ${uuid}"
            echo -e "Port: ${port}"
            echo -e "域名: niboy0088.niboy.top"
            echo -e "VLESS节点信息: vless://${uuid}@niboy0088.niboy.top:${port}?flow=&security=none&encryption=none&type=ws&host=niboy0088.niboy.top&path=/&sni=&fp=&pbk=&sid=#$USER.serv00.vless"
